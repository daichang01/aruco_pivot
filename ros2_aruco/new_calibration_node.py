...
class ArucoNode(rclpy.node.Node):
    ...
    def caculate_relative_pose(self):
        # 八棱柱边长和水平距离
        edge_length = 0.022  # 22mm
        radius = 0.022/ (2 * np.tan(np.pi / 8)) 
        print(radius)

        # 八个Aruco码的ID及相对八棱柱中心的位姿
        aruco_relative_poses = {}

        for i in range(8):
            aruco_id = 10 + i  # Aruco码ID
            theta = np.deg2rad(45 * i)  # 每个码的角度，单位为弧度
